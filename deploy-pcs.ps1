...
# 1) Folders
$dirs = @(
  "app","assets","configs","gallery\images","gallery\thumbs","models\lora","models\lcm","sprites","hf_cache"
) | % { Join-Path $root $_ }
$dirs | % { New-Item -ItemType Directory -Force -Path $_ | Out-Null }

# 2) Requirements (small)
$req = @'
gradio>=4.35.0
diffusers>=0.30.0
transformers>=4.43.0
...
'@
Set-Content -Encoding UTF8 -Path (Join-Path $root "requirements.txt") -Value $req

# 3) Curated presets (portable paths under models\)
$curated = @'
{
  "presets": [
    {
      "name": "SDXL + PixelArtXL",
      "base_model": "stabilityai/stable-diffusion-xl-base-1.0",
      "refiner_model": "stabilityai/stable-diffusion-xl-refiner-1.0",
      "lcm_dir": "models/lcm",
      "loras": [
        { "path": "models/lora/pixel-art-xl.safetensors", "weight": 1.0 }
      ],
      "suggested": { "mode": "Quality (Base)", "steps": 30, "guidance": 7.0 }
    }
  ]
}
'@
Set-Content -Encoding UTF8 -Path (Join-Path $root "configs\curated_models.json") -Value $curated
...
# 5) Minimal app (path-agnostic, CPU-friendly)
$app = @'
...
# 6) Start shortcut (double-clickable)
$bat = @'
@echo off
setlocal
...
# 7) Python venv + deps (CPU default; change if you want CUDA/DML)
