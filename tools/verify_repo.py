...
for t in ["tools/test_presets.py","tools/check_migration.py","tools/sanitize_reports.py"]:
    checks.append(expect_file(t, t))
checks.append(expect_file("configs/curated_models.json", "Curated presets"))
for d in ["loras","outputs"]:
    full = os.path.join(ROOT,d)
    print(f"[OK] Directory {d} exists" if os.path.isdir(full) else f"[FAIL] Directory {d} missing")
if not all(checks):
    sys.exit(1)
