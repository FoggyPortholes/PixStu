        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if ERROR_PATTERNS["dtype_error"].search(msg):
                    log_recovery(name, e, "Forced dtype=float32; retry once")
                    kwargs["dtype"] = "float32"
                    return func(*args, **kwargs)
                if ERROR_PATTERNS["preset_format"].search(msg):
                    log_recovery(name, e, "Ignored malformed preset; returned empty set")
                    return []
                if ERROR_PATTERNS["sqlite_corrupt"].search(msg):
                    ...
                    try:
                        ...
                        log_recovery(name, e, "Cache DB recreate failed")
